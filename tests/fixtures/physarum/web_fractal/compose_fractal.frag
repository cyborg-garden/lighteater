...
// if kept. New for the fractal veins demo (no vendored counterpart), loaded
// only when the fractal amount is above 0.
//
...
//
// u_mix < 1 blends toward the stock flat sum (the vendored tonemap's lum),
// so small amounts still morph out of today's picture.
...
vec3 trailLin(vec2 gp) {
    vec2 q = gp - 0.5;
    vec2 fr = fract(q);
    ivec2 hi = u_grid - 1;
    ivec2 c0 = clamp(ivec2(floor(q)), ivec2(0), hi);
    ivec2 c1 = clamp(c0 + 1, ivec2(0), hi);
    vec3 a = texelFetch(u_trail, c0, 0).rgb + u_grain * u_inv_gnorm / u_inv_norm * texelFetch(u_laid, c0, 0).rgb;
    vec3 b = texelFetch(u_trail, ivec2(c1.x, c0.y), 0).rgb + u_grain * u_inv_gnorm / u_inv_norm * texelFetch(u_laid, ivec2(c1.x, c0.y), 0).rgb;
    vec3 c = texelFetch(u_trail, ivec2(c0.x, c1.y), 0).rgb + u_grain * u_inv_gnorm / u_inv_norm * texelFetch(u_laid, ivec2(c0.x, c1.y), 0).rgb;
    vec3 d = texelFetch(u_trail, c1, 0).rgb + u_grain * u_inv_gnorm / u_inv_norm * texelFetch(u_laid, c1, 0).rgb;
...
void main() {
    ...
    if (u_mix < 1.0) {
        vec3 t = trailLin(gp);
        float lumS = 1.0 - exp(-u_exposure * (t.r + t.g + t.b) * u_inv_norm);
        outL = mix(lumS, outL, u_mix);
    }
