...
// if kept. A modified copy of vendor/physarum/tonemap.frag for the fractal
...
// compose_fractal.frag then draws those outlines at SCREEN size, so every
...
uniform float u_fractal;   // 0..1, carried for the stock blend in compose
...
void main() {
    ...
    xr += xs.r - x.r;
    // (divided by 1 - share/2, so a lone trunk comes out as bright as before)
